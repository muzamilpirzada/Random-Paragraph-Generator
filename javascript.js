...
function generate(){
    if(isNaN(item.value) || item.value < 0 || item.value>9){
        ...
        dataList.innerHTML =`<p>${text[randomIndex]}</p>`
    }else{
        const data =text.slice(0,item.value);
        const suffleArray =shuffle(data);
        const paras =suffleArray.map( // this is used for the one to one paragraph display on DOM as a Text.
            (d)=>{
               return `<p>${d}</p>`
            }
        )
        dataList.innerHTML = paras.join(""); // join method is used for Array to string conversion.
    }
